...
function loadDataIntoBST() {
  ...
  // Save the serialized BST in ScriptProperties
  PropertiesService.getScriptProperties().setProperty('BST_DATA', bstJSON);

  const endTime = Date.now(); // End timing
  const loadTime = ((endTime - startTime)).toFixed(2); // Calculate loading time in seconds

  SpreadsheetApp.getUi().alert(`Data has been successfully loaded into the Binary Search Tree!\nLoading Time: ${loadTime} mili seconds`);
}

function searchKeyword() {
  const ui = SpreadsheetApp.getUi();

  // Retrieve the serialized BST from ScriptProperties
  const bstJSON = PropertiesService.getScriptProperties().getProperty('BST_DATA');
  ...
  const response = ui.prompt('Enter the keyword to search:');
